...
export async function loadAccessibleQuiz(
...
export function resolveFacultyRoll(
  user: AuthTokenPayload,
  bodyFacultyRoll?: string,
): string {
  ...
  if (user.role === "admin") {
    if (!bodyFacultyRoll) {
      throw new ApiError(
        400,
        "facultyRoll is required when an admin creates or lists quizzes on a faculty member's behalf",
      );
    }
